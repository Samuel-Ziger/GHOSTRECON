...
export async function runForgeCorrectionLoop({
  ...
  for (let attempt = 1; review?.status === 'changes_requested' && attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      const generated = await generateImpl({ provider, request, target, root, pendingDir, attempt, env, execFileImpl });
      const gates = await validateImpl(pendingDir, { env });
      review = gates.ok
        ? await reviewImpl({ pendingDir, root, providers, env, fetchImpl, execFileImpl })
