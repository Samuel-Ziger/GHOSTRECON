...
For an orchestrated authenticated scan, CLI v1 can export the captured session
to a mode-`0600` JSON file and pause before scanning:

```bash
frameseven \
  -url https://target.example \
  -auth-browser \
  -auth-session-out /protected/runtime/session-v1.json \
  -auth-wait-after-capture
```

The file contains only captured cookies, authentication headers, and same-origin
endpoints. It never contains the username or password entered in the browser.
The orchestrator must delete it after completion, cancellation, or timeout.
